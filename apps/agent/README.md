...
- `agent/lib/` — composition helpers: hosted Better Auth session verification, loopback-only local owner auth, owner resolution, Redis-backed ingress limits, the Google Calendar reader (`calendar.ts` / `calendar-read.ts`, reading the connected calendar through the shared cache-aside seam; token custody stays in Better Auth), and the background-job enqueue/publish wiring.
- `agent/skills/` — Markdown playbooks: `capturing-and-review.md`, `recall.md`, `followups.md`, `drafting.md`.
- `agent/channels/eve.ts` — the same-origin HTTP channel and hosted trust boundary; verifies Better Auth cookies directly, requires admitted Private Beta Access, charges the Eve ingress budget, and stamps the verified owner onto the session principal (ADR 0194).
