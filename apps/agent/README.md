...
- `agent/tools/` — typed tools over the owner-scoped `@tendnote/db` queries: people search/create/update, source-record and memory capture, suggested-memory and suggested-follow-up review, follow-up lifecycle, Exact Recall and semantic search, the relationship agenda, and Tendnote-only message drafting.
