 ...
 */
function currentDateMarkdown(now: Date, timeZone: string): string {
 ...
 *
 * A date anchor is worth more than a perfect one: an unauthenticated, runtime, or
 * child-agent session has no owner to read a zone for, and a projection failure
 * must not cost the turn its only knowledge of what day it is. Both fall back to
 * UTC, which is exactly what this resolver used to hand every caller.
 *
 * The caller check is the orientation one on purpose. It is stricter than a date
 * anchor needs — it also refuses child-agent sessions — but reusing it keeps one
 * definition of "a directly authenticated human owner" in the agent, and being
 * refused here costs only the zone, never the date.
 */
async function resolveOwnerTimeZone(ctx: Parameters<typeof resolveOrientationCaller>[0]) {
  const ownerUserId = resolveOrientationCaller(ctx);
...
export default defineDynamic({
  events: {
    ...
    "turn.started": async (_event, ctx) =>
      defineInstructions({
        markdown: currentDateMarkdown(new Date(), await resolveOwnerTimeZone(ctx)),
      }),
  },
});
