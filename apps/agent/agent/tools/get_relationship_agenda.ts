...
export default defineTool(
  relationshipAgendaTool({
    description:
      "Read the user's visible relationship agenda for broad questions like 'anything coming up next week?', 'who deserves a thought today?', 'who should I prioritize?', or 'what follow-ups are due soon?'. Visible context includes the caller's private records plus selected-member and whole-household records the caller can view. The root agent may use this directly for lightweight read-only agenda and prioritization answers; use relationship_strategist for deeper synthesis or review-gated follow-up proposals. Pass a concrete windowStart/windowEnd, optional query, limit, and includeKinds. Restricted candidates are withheld unless the user explicitly asked for them; `directlyRequested` is how you ask, and only that argument puts the call in front of the user. If they decline, work from the ordinary candidates instead of asking again. The ordinary agenda read needs no permission — run it and answer. This is read-only agenda ranking over existing context; never use it to create reminders, suggestions, scans, or brief artifacts. Preserve visibility/provenance language when it affects trust or actionability. Return people by display name and never show raw ids.",
