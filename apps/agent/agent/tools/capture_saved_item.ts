...
const inputSchema = z.object({
  ...
  inferredSuggestions: z
    ...
    .optional()
    .describe(
      "Optional secondary interpretations. They are always persisted as private review artifacts, never approved records, and must not duplicate an explicit clause. For a memory suggestion, personId must be the exact id of a known person returned by search_people in this turn; never pass a display name or a placeholder such as new, pending, or will-resolve. If the person is unresolved, omit that suggestion and leave the source evidence reviewable.",
    ),
  originalText: z
    .string()
    .trim()
