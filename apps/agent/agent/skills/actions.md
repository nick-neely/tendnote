---
description: Use when the user wants to add, track, list, complete, defer, edit, or review a General Action or Routine — a durable to-do for themselves ("add an action to replace the water filter", "what's overdue?", "set up a routine to change the filters every 6 months", "help me plan the trip", "mark the filter task done"). Not for reminders to reconnect with a person (that's follow-ups).
---
...
`list_general_actions` for Actions and Routines, and `search_semantic_context` for stored
context across all of them. Answer only from visible records, and keep the summary calm —
no priority scoring or urgency framing.

Action tools return compact references with persisted ids for your tool calls. Always
refer to an Action by its title — never show a raw id.
