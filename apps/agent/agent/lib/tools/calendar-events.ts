...
import { z } from "zod";
import { createOwnerCalendarReader } from "../calendar";
import { runCalendarRead } from "../calendar-read";
...
type CalendarOutput = Awaited<ReturnType<typeof calendarRead.execute>>;

/**
 * The Calendar read as both the root agent and `relationship_strategist` register
 * it.
 *
 * Nothing about this read differs by caller: the same bounded window, the same
 * minimized events, the same "provider-derived, not saved memory" framing, and the
 * same standing refusal to write to Calendar. The two hand-copied files had drifted
 * anyway - the subagent's had collapsed every field description and dropped the
 * cannot-RSVP and not-connected sentences - so there is one description here and no
 * option to vary it.
 */
export function calendarEventsTool() {
  return {
    description:
    ...
    toModelOutput(output: CalendarOutput) {
