...
export default defineEval({
  ...
  async test(t) {
    const now = new Date();
    const createdDate = futureFixtureDate(7, now);
    const snoozedDate = futureFixtureDate(14, now);
    const createdDueDate = new RegExp(`^${createdDate}`);
    ...
    await t.send(`Snooze the Dana ops-notes follow-up to ${snoozedDate}.`);
    ...
    t.calledTool("update_followup_status", {
      input: { followupId: UUID, status: "snooze", dueAt: snoozedDueDate },
    });
    t.eventsSatisfy(
