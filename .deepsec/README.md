...
Currently configured project: `tendnote` (target: `..`).
...
original init command. Checkpoints in `data/tendnote/setup/setup-state.json`
...
`--project-id tendnote` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/tendnote/`.
