# Agent setup for `tendnote`

This is a deepsec scanning workspace. Project `tendnote` was just registered
(target: `..`). Setup is incomplete — `data/tendnote/INFO.md`
...
2. **Fill in `data/tendnote/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id tendnote
pnpm deepsec process --project-id tendnote
```
