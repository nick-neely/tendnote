...
Optimistic and pending interactions follow ADR 0209. Direct user writes return an authoritative view and revision or mutation marker only after commit and synchronous scope invalidation. Clients retain newer acknowledged state when an older cached render arrives, while an incompatible concurrent mutation fails visibly and reconciles to the latest server view.
