...
Household Invitations are the one email Tendnote sends. Locally you do not need a provider: with no `RESEND_API_KEY` set, the whole message — subject, body, and the acceptance link — is written to the `next dev` terminal, so you can walk the recipient's side by pasting the link into a browser. Setting `RESEND_API_KEY`, `TENDNOTE_EMAIL_FROM`, and an explicit `TENDNOTE_EMAIL_REPLY_TO` turns real sending on in any environment, which is how you smoke-test one send; `vitest` never sends whatever is configured. Production without any required value refuses by name rather than writing a live capability URL into a hosted log. Sending domain, DNS records, and the send checklist: [`email-setup.md`](email-setup.md).
...
`pnpm verify` does **not** run Fallow, but every code pull request does, so a clean local `verify` can still fail the PR gate. The `Test and Fallow` job runs `pnpm coverage:ci` (V8 coverage, which Fallow consumes), then `pnpm fallow:coverage:check`, then `pnpm fallow:ci`. To see what that gate sees before pushing:
...
- `.github/workflows/reusable-verify.yml` runs the selected lanes in parallel. Vercel owns the deployable production build.
  ...
  - **Instant matrix** (when Instant paths change) - routine Chromium coverage, widened to the promotion browser matrix on request.
...
- `.github/workflows/promotion-verify.yml` runs the same verification definition as a pull request, with the Instant Interaction browser matrix widened to add the reduced Firefox and WebKit smoke; it only runs via `workflow_dispatch` or the `full-browser-matrix` PR label, so it never gates a routine PR.
...
- The tracked [default-branch ruleset](../.github/rulesets/protect-main.json) records the intended one-code-owner-approval, per-job verification, and Vercel requirements before merge. It is source-controlled publication intent, not proof that the live GitHub settings currently enforce those checks; live default-branch protection and CLA Assistant enforcement remain pending #473. Workflow files, `.github/runs-on.yml`, and the ruleset source are owned by `@nick-neely` in [CODEOWNERS](../.github/CODEOWNERS). After merge, Vercel sends `.github/workflows/production-migrations.yml` a `vercel.deployment.ready` repository dispatch for the staged production build. The workflow validates the event payload and configured project first: previews and non-`main` builds are explicitly ignored, while a production/main/ready event with a missing, malformed, or mismatched `VERCEL_PROJECT_ID` fails loudly in GitHub instead of silently skipping the migration job. Invalid events do not publish a Vercel status because the status action keys commit statuses by SHA and context; that prevents a different Vercel project sharing the same commit from overwriting the target project's gate. A matching production/main/ready event then idempotently applies any pending Drizzle migrations using a database-only dependency install. Running the migration ledger on every ready deployment avoids missing schema work if Vercel coalesces multiple `main` builds. It reports the stable `Production Release Gate` commit status back to Vercel and expects `PRODUCTION_DATABASE_DIRECT_URL` in the production GitHub environment. Configure the repository variable `VERCEL_PROJECT_ID` to the operator's Vercel project id; the workflow deliberately does not embed a maintainer-owned project id. The RunsOn stack is the same-owner `.github-private` extension declared in `.github/runs-on.yml`.
