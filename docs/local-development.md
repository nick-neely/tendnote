...
- `.github/workflows/pr-verify.yml` is the pull request wrapper and always publishes the stable `Verify` check. Draft PRs run the affected-test fast tier. Apply the `full-ci` label to run full qualification while a PR is still in draft; ready-for-review code PRs must retain that label before merge.
