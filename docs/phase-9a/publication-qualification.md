...
The command refuses a different checkout or a different top-level/nested input
SHA. Without an input file it emits a blocked report with every gate `not-run`
and explicit boundary blockers; that default is deliberately not useful as a
release shortcut.
...
| Deterministic evidence integrity | `apps/agent/scripts/package-deterministic-evidence.mjs` output and `verifyDeterministicEvidenceBundle` | The clean `raw/initial-summary.json`, `raw/initial-results.jsonl`, JUnit, metadata, source SHA, and SHA-256 files agree; retry artifacts are rejected. |
...
and read-only verification actually occur. Each criterion is machine-bound in
the report to the exact issue or setting below; a free-form `passed` boolean or
status cannot qualify it:
