...
- Capture is save-first. Before asking a consequential clarification, Tendnote persists the original input as a Pending Source Record so abandoning the exchange cannot lose it. Missing reminder timing prompts one focused question, while an unscheduled General Action remains valid; deterministic phrases such as `tomorrow` or `Friday` resolve in the owner's timezone, while genuinely vague timing does not become an invented date.
...
- Successful Reminder Opt-In covers every notification-eligible record the owner can see on that installation after deliberately creating or accepting it. Phase Seven does not add per-record push toggles or a notification-preference matrix; the backing record's due time and lifecycle remain the controls, and an inferred suggestion cannot notify until the owner accepts it into an active time-bound record.
