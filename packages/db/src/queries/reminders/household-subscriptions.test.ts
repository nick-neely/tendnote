...
describe("dispatch-time identity", () => {
  it("is the subscriber, not the record owner", async () => {
    ...
    //
    // The recheck reads `subscriptionAuthorized`, which is #383's injected proof
    // for Actions and Routines and this identity comparison for everything else.
    // Both halves are pinned: the fallback must stay subscriber-keyed, and no
    // path may go back to comparing the record's owner to the claimant.
    const source = await import("node:fs").then((fs) =>
      fs.readFileSync(new URL("./dispatch.ts", import.meta.url), "utf8"),
    );
    expect(source).toContain("!context.subscriptionAuthorized");
    expect(source).toContain("reminderSubscriber(record) === claimed.ownerUserId");
    expect(source).not.toContain("record.ownerUserId === claimed.ownerUserId");
