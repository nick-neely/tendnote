...
function buildPrompt(input: SuggestedActionExtractionInput) {
  return [
    ...
    "Each candidate needs a short imperative title. Optionally add: a one-line source-grounded reason, a due date, a defer date, a simple recurrence (interval + unit for a recurring chore), asset hints (plain subject labels like 'refrigerator water filter'), person links (only ids from the list below), an area id (only from the list below), and scope (private or household).",
    "Preserve explicit urgency or difficulty wording naturally in the reason when it helps explain the proposal. Do not invent or normalize classifications that are not stated in the source.",
