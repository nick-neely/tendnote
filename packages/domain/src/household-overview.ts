 ...
 * in it, what the reader's own role is, and how much of its capacity is spoken
 * for. It carries no invitation, audit, or other-member private state.
 */
 ...
 * identities.
 *
 * Two invariants are load-bearing. Only `active` memberships are described, so
 * an invited or removed row never reads as a member or occupies a seat. And the
 * seat count is derived from those memberships rather than from the joined
 * identities, so an incomplete identity read shows fewer rows instead of
 * understating how full the household is.
 */
export function buildHouseholdOverview(input: {
  ...
  return {
    ...
    seats: householdSeatUsage({
      ...
      liveInvitations: input.liveInvitations,
    }),
